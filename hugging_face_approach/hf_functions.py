...
def fix_beginnings(labels):
    """
    is this not circular, self-referential and confusing? we have labels as an input and label as an output. obviously
    we're doing something to it in the function. I think we should rename it to new_label and old_label
    :param labels:
    :return labels:
    """
...
def tokenize_and_align_labels(examples, max_length, tokenizer, labels_to_index, stride):
    ...
    for i in range(len(offset_mapping)):
        ...
        for label_start, label_end, label in list(zip(examples['starts'][sample_index], examples['ends'][sample_index],
...
def compute_metrics(p, index_to_label, metric):
    ...
    true_predictions = [[index_to_label[p] for (p, l) in zip(prediction, label) if l != -100] for prediction, label in
                        zip(predictions, labels)]

    true_labels = [[index_to_label[l] for (p, l) in zip(prediction, label) if l != -100] for prediction, label in
                   zip(predictions, labels)]
...
def tokenize_for_validation(examples, tokenizer, label_to_index, fix_beginnings):
    ...
    for i in range(len(offset_mapping)):
        ...
        for label_start, label_end, label in list(
                zip(examples['starts'][i],
                    examples['ends'][i],
                    examples['classlist'][i]
                    )
        ):
...
def pred_to_span(text, all_span, classes, visualize, min_tokens, viz=False):
    ...
    # mk: short spans are likely to be false positives, we can choose a min number of tokens based on validation
